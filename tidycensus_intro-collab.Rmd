...
iowa_foreign_born_population$NAME <- str_replace(iowa_foreign_born_population$NAME, "O'Brien", "OBrien")
```
...
foreign_born_2008_2018 <- map_dfr(years, ~{
  get_acs(
    geography = "state",
    variables = c(percent_of_foreign_born_population_by_household = "DP02_0109P"),
    state = "IA",
...
ggplot(foreign_born_2008_2018, aes(x = year, y = estimate, group = 1)) + 
  geom_line() + 
  geom_point()
```
...
The map in Visual 1 shows the percent of foreign born population across Iowa. The darker areas represent a lower percent of the population that is foreign born, while the lighter areas represent a higher percent of the population that is foreign born. The vast majority of Iowa has under 50% of the population that foreign born, very few counties have above 50%. Some data is not applicable for several counties.

Visual 2 is a time series map which shows the estimated percentage of the foreign-born population in Iowa overtime. From 2008-2018 there are some peaks and lows but overall the percentage as gone down. The largest foreign-born population percentage occurred in 2010 at about 3.6% and the lowest in 2017 at 1%.

Visual 3 is the same time series map with the addition of margin of error (shown in blue), which is about 1%. While the margin of error seems low, in proportion with the data it is high as all the data ranges between 1-3%.

The scatter plot in Visual 4 displays a comparison of the percent of foreign born population by household to the percent of individuals with bachelors degrees or higher in Iowa. The correlation between the two variables is much closer to 0 than 1, which means there is little to no statistical significance that the percent of foreign born population in an area has any impact on the percent of individuals with bachelors degrees or higher in that same area.
